...
With the growing number of structured data published on the Web, WWW is moving towards becoming a rich ecosystem of machine-understandable Linked Data\footnote{\url{lodlaundromat.org} recently (14.10.2015) reported approx. 38.6 billion triples published on the Web.}.
...
As shown below, in the worst case, when the \texttt{DRPV} scopes are given and the UI is supposed to render the Value components, all the 15 scopes need to be traversed for the adaptation:
...
We believe that making a bridge between Semantic Web and Web Components worlds brings mutual benefits for both sides.
On one hand, the Semantic Web technologies provide support for richer component discovery, interoperability, integration and adaptation on the Web.
On the other hand, Web Components bring the advantages of UI standardization, reusability, replaceability and encapsulation to the current Semantic Web applications.
